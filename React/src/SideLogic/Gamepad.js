...
function ControlData({onJoystick}) {
...
function AxeData({onAxe}) {
    ...
    function ProcessAxe(array) {
        ...
        else if (array[0] !== 1 && array[1] !== 1) onAxe("center")
    }
...
const Gamepad = () => {
    ...
    const { setAxe } = useContext(SystemContext)
    const { setButton } = useContext(SystemContext)

    function GetAxeData(onAxe) {

        setAxe(onAxe)
    }

    function GetButtonData(onButton) {
    ...
    return (
        ...
        <div>
            <AxeData onAxe={GetAxeData}/>
            <ButtonData onButton={GetButtonData}/>
        </div>
    )
}
